...
typedef struct _data_split {
  ...
  void* usr_data; /* This field is used only by the "Word finder" program: it
                     records the word to find in the input data file */
} DATA_SPLIT;
...
typedef struct _mapreduce_spec {
  ...
  int (*map_func)(
      DATA_SPLIT* split,
      int fd_out); /* Function pointer to the user-defined map function */
  int (*reduce_func)(
      int* p_fd_in, int fd_in_num,
      int fd_out); /* Function pointer to the user-defined reduce function */
  void* usr_data;  /* This field is used only by the "Word finder" program: it
                      records the word to find in the input data file */
} MAPREDUCE_SPEC;
