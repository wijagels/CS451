 ...
 * @return: the position in the bitmap of the first 0 bit (if "val" is zero) or
 * the first 1 bit (if "val" is non-zero)
...
int bitmap_set_bit(unsigned char* bitmap, int size, int target_pos) {
    if (size == 0 || target_pos > size * BIT_PER_BYTE) return BITMAP_OP_ERROR;
    int idx = target_pos / BIT_PER_BYTE;
    unsigned char offset = target_pos % BIT_PER_BYTE;
    unsigned char mask = 0x01 << offset;
...
int bitmap_bit_is_set(unsigned char* bitmap, int size, int pos) {
    if (size == 0 || pos > size * BIT_PER_BYTE) return BITMAP_OP_ERROR;
    int idx = pos / BIT_PER_BYTE;
    unsigned char offset = pos % BIT_PER_BYTE;
    unsigned char mask = 0x01 << offset;
    return (bitmap[idx] & mask) >> offset;
}
