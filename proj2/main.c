...
void print_usage(char* cmd_name) {
    printf(
        "Usage: %s \"counter\"|\"finder\" file_path split_num [word_to_find]\n",
        cmd_name);
}
...
int main(int argc, char* argv[]) {
    ...
    result.filepath =
        "mr.rst";  // name of the output file (placed in the working directory)
    result.map_worker_pid =
        malloc(spec.split_num * sizeof(*result.map_worker_pid));
    ...
    for (i = 0; i < spec.split_num; i++)
        printf("%d ", result.map_worker_pid[i]);
    printf("\n");
